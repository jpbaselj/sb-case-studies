...
the member name. */
SELECT DISTINCT f.name, CONCAT(firstname,'_',surname) AS membername
FROM Bookings as b
...
SELECT f.name, CONCAT(firstname,'_',surname) AS membername,
...
FROM revenues
GROUP BY facility
HAVING revenue < 1000 ORDER BY revenue DESC
...
SELECT b.facid as Facility,f.name, COUNT(b.memid) AS usage_
FROM (
	SELECT facid, memid
	FROM Bookings
	WHERE memid !=0
...
SELECT b.month, COUNT( b.memid ) AS usage_
FROM (
